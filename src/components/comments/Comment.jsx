...
const replyOnUserId = comment.user._id;
  return (
    ...
    <img src={comment?.user?.avatar ? stables.UPLOAD_FOLDER_BASE_URL+comment.user.avatar : images.sampleProfileImage} alt="user profile"  className='w-9 h-9 object-cover rounded-full'/>
...
<div className='flex items-center gap-x-3 text-dark-light font-roboto text-sm mt-3 mb-3'>
    {isUserLoggined && (
    <button className='flex items-center space-x-2' onClick={()=> setAffectedComment({type: "replying", _id: comment._id, replyOnUserId})}>
...
{commentBelongsToUser && <>
    <button
        ...
        onClick={()=> setAffectedComment({type: "editing", _id: comment._id, replyOnUserId})}
        >
 ...
 isReplying && (<CommentForm
     btnLabel="Reply"
      formSubmitHandler={(value)=>
        addComment(value, repliedCommentId)
    }
formCancelHandler={()=> setAffectedComment(null)}
/>
 )}
